* OpenFlow と Trema 入門
** はじめに
2013 年は OpenFlow がバズワードのごとく急激に浸透した年でした．たしかに
出版業界やネットでは OpenFlow の解説記事をよく見かけます．しかし "ネッ
トワークをソフトウェアで制御できる" というよく聞くメリットや，解説記事
にいきなり出てくる "コントロールプレーン" "タプル" などの専門用語のせい
で，とくにネットワークが専門外の人達にとって分かりづらいという声も聞き
ます．

しかし派手な宣伝文句や難解な専門用語を取り除いてしまえば，その基本的な
仕組みはいたってシンプルです．ひとことで言えば OpenFlow とは "ネットワー
クスイッチを制御するための標準プロトコルの一種"，これだけで
す[fn:onf]. 最近増えつつある OpenFlow に対応したスイッチであれば, この
標準化されたプロトコルを通じてその内部動作を変更できます.

変更できる内部動作はスイッチの根本的な機能にまで及びます．標準とは異な
る動作をスイッチにさせたい場合，従来はファームウェアの書き換えが必要で
した．しかし OpenFlow を使えば，OpenFlow 対応スイッチをロードバランサや
ファイヤウォールに変身させるのはもちろん，目的に応じてパケット転送をい
かようにでも制御できてしまいます．

こうした特徴は実は，ネットワーク管理者だけでなくサーバ管理者にとっても
うれしいものなのです．たとえば従来のスイッチでは, 同じ IP アドレスを持
つサーバをつなげると正しく通信できませんでした. このためロードバランサ
のバックエンドサーバなど，構成はほぼ同じで IP アドレスだけが違うサーバ
群の管理は面倒でした．ところが OpenFlow では, IP アドレスが同じでも通信
できる特別なスイッチを作ることができます．このおかげで，たとえばひとつ
のサーバー構築スクリプトだけでバックエンドサーバーをいくつでも増やせます．

(ここまで校正した)

さらにうれしいことは, プログラミング言語が自由に選べることです.たとえば
RFC で標準化された HTTP プロトコルは, 好きな言語で実装できます.

これがいわゆる SDN (Software Defined Network), つまり「ネットワーク機器
をプログラミングする」というプログラミングの新ジャンルです.

さらに, OpenFlow 対応スイッチでネットワークを組めば, ネットワーク全体を
ソフトウェアで制御できるようになります.
