...
しかし流行があまりにバズワード的だったため, きちんと理解されていないことも事実です. そもそも解説記事でよく目にする 「OpenFlow はなんでもできる」という売り文句があいまいでよく分かりませんし，話題に追い付こうとする間にも新しい専門用語がどんどん登場します. さらに「大手○○社も OpenFlow に参入!」のような連日のトップ記事からの情報もごっちゃになって,もう何が何だか…という人も多いでしょう.

*** 基本はシンプル!
実は派手な宣伝文句や専門用語を取り除けば，OpenFlow の基本はいたってシンプルです．「OpenFlow とはネットワークスイッチを制御するための標準プロトコルの一種である」とひとことで説明できるからです[fn:onf]. 最近増えつつある OpenFlow に対応したスイッチであれば, この標準化されたプロトコルを通じてその内部動作を変更できます.

OpenFlow では, このスイッチをコントロールするソフトウェアをコントローラと呼びます. コントローラは OpenFlolw プロトコルを実装したプログラムの一種で, 何台もの OpenFlow スイッチをまとめて制御できます ([[fig:openflow_controller]]).よく聞く「SDN ではソフトウェアでネットワークを定義できる」「分散ではなく集中的に管理できる」などのうたい文句はこの仕組みから来ています.

#+CAPTION: OpenFlow スイッチとコントローラの関係
#+NAME: fig:openflow_controller
[[./images/openflow_controller.png]]

コントローラが変更できるスイッチの内部動作は, スイッチの根本的な機能にまで及びます．標準とは異なる動作をスイッチにさせたい場合，従来はファームウェアの改造が必要でした．しかし OpenFlow を使えば，スイッチのパケット転送ルールを書き換えてロードバランサやファイヤウォールに変身させるなど，基本レベルから動作をいかようにでも変更できてしまいます．

[fn:onf] OpenFlow の詳しい仕様書や標準化に関する情報は、Open Networking Foundation のサイト (https://www.opennetworking.org/) で入手できます。

*** 「なんでもできる」メリット
この「なんでもできる」という特徴は実は，ネットワーク管理者よりもむしろサーバ管理者にとってうれしいものです．たとえば従来のスイッチでは, 同じ IP アドレスを持つサーバをつなげると正しく通信できませんでした.このためロードバランサのバックエンドサーバなど，構成は同じで IP アドレスだけが違うサーバ群の構築はちょっと面倒でした．ところが OpenFlow では, IP アドレスが同じでも通信できる特別なスイッチを作ることができます．このおかげで，たとえばひとつのサーバー構築スクリプトだけでバックエンドサーバーをいくつでも増やせます．

プログラマにとっての利点もあります. OpenFlow は結局, ただのプロトコル仕様なので, プログラミング言語を選ばず好きな言語で実装できます. たとえば後に紹介する Trema は Ruby ですし, そのほかにも Java や Python, Haskell など主要な言語バインディングがすでに揃っています. プログラマやチームの好みに合わせて, 使う言語を自由に選べるのです. これはプログラマにとって, 「ネットワーク自体をプログラミングできる」という新しい仕事 (チャンス) が増えたことを意味します.

*** すべてを連携する
さまざまな言語の豊富なライブラリが使えるので, OpenFlow で作ったネットワークは様々なソフトウェアと簡単に連携できます. たとえばスイッチの DB に memcached や sqlite を使ったり, ユーザーアカウンティングサービスなどのミドルウェアと連携して動作するネットワークを作ることも簡単です.

このように, OpenFlow を使えば IT システム全体をソフトウェアで制御できるようになります.たとえば Google などの巨大データセンターではすでにミドルウェアで大量のサーバを制御していますから, OpenFlow でネットワークも連携すればデータセンター全体をすべてソフトウェアで制御できることになります.Google が提唱した「コンピューターとしてのデータセンター」のように,データセンター全体をひとつのコンピュータとしてプログラミングできる世界,それが SDN です.

話が少し大規模になりましたが, 続く解説では小さな規模でも OpenFlow プログラミングをすぐに試せる構成になっています. 職場や自宅のような中小規模ネットワークでもすぐに試せる実用的なコードを通じて，「OpenFlow ってどんな仕様？」「どうやってプログラムを書けばいいの？」という疑問に答えていきます。ネットワークの基礎から説明しますので，ネットワークの専門家だけでなく，普通のプログラマもすんなり理解できるはずです.
...
HTML と Web サイトの関係を見てください. Web サイトを作るには,
W3C が標準化する HTML を使って Web ページを作ります. プログラマが使う言
語に対応した Web フレームワーク (Rails や Django など) を使います.
