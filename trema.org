* Trema と OpenFlow 入門
** はじめに
ネットワーク業界では OpenFlow が新しいバズワードとして注目を集めていま
す. 当たり外れ含めて OpenFlow を取り上げた記事が急激に増えたことや,「ネッ
トワークをソフトウェアで制御できる」というOpenFlow のメリットが分かりづ
らいこともあって, とくに専門外の人にとってそのイメージがつかみづらくなっ
ています.

OpenFlow を正しく一言で説明すると, 「ネットワークスイッチを制御するため
の標準プロトコルの一種」となります[fn:onf]. OpenFlow プロトコルに対応し
たスイッチであれば, この標準プロトコルを通じて内部動作を自由に変更でき
ます.

このうれしさのひとつは,既存のネットワークの制約を受けないことです. たと
えば従来のレイヤ 2 スイッチでは, 同じ IP アドレスを持つサーバを複数つな
げると正しく通信できませんでした. しかし OpenFlow では, パケットの様々
な特徴を使って送り先を切り替えられるので, 同じ IP アドレスであっても

もうひとつは, プログラミング言語が自由に選べることです.たとえば RFC で
標準化された HTTP プロトコルは, 好きな言語で実装できます.
