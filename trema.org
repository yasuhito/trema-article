...
しかしはやりかたがあまりにバズワード的だったため, きちんと理解されていないことも事実です. そもそも解説記事でよく目にする 「OpenFlow はなんでもできる」という売り文句があいまいでよく分かりませんし，話題に追い付こうとする間にも新しい専門用語がどんどん登場します. さらに「大手○○社も OpenFlow に参入!」のような連日のトップ記事からの情報もごっちゃになって,もう何が何だか…という人も多いでしょう.
...
*** 本当のメリット
この「なんでもできる」という特徴は実は，ネットワーク管理者よりもむしろサーバ管理者にとってうれしいものです．たとえば従来のスイッチでは, 同じ IP アドレスを持つサーバをつなげると正しく通信できませんでした.このためロードバランサのバックエンドサーバなど，構成は同じで IP アドレスだけが違うサーバ群の構築はどこか面倒でした．ところが OpenFlow では, IP アドレスが同じでも通信できる特別なスイッチを作ることができます．このおかげで，たとえばひとつのサーバー構築スクリプトだけでバックエンドサーバーをいくつでも増やせます．
...
それぞれの言語の豊富なライブラリが使えるので, OpenFlow で作ったネットワークと様々なソフトウェアを簡単に連携できます. たとえば memcached や sqlite などをスイッチの DB に使ったり, Hadoop などのミドルウェアと連携して動作するネットワークを作ることもできます.

これがいわゆる SDN の本質です. OpenFlow 対応スイッチでネットワークを組
めば, ネットワーク全体をソフトウェアで制御できるのはもちろんです. さら
に既存のツールやミドルウェアと連携した高度なネットワークを作れることが
OpenFlow の強みです. Google などの巨大データセンターではたくさんのミド
ルウェアがすでに動作していますから, OpenFlow でネットワークと連携すれば
データセンター全体をすべてソフトウェアで制御できることになります.

ネットワークをソフトウェアとして記述することにより，たとえば「アプリに合わせて勝手に最適化するネットワーク」や「障害が起こっても自己修復するネットワーク」といっ
た究極の自動化も夢ではなくなります！
